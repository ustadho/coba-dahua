...
dahua.prototype.connect = function (options) {
  ...
  client.on("socket", function (socket) {
    ...
    if (TRACE)
      console.log("TCP_KEEPINTVL:", NetKeepAlive.getKeepAliveInterval(socket));
...
dahua.prototype.createFileFind = function () {
  var self = this;
  request(
    BASEURI + "/cgi-bin/mediaFileFind.cgi?action=factory.create",
    function (error, response, body) {
      if (error) {
...
dahua.prototype.getLog = function (start, end) {
  var self = this;
  request(
    `${BASEURI}/cgi-bin/log.cgi?action=startFind&condition.StartTime=${start}&condition.EndTime=${end}`,
    function (error, response, body) {
      if (error) {
...
dahua.prototype.startFileFind = function (
  objectId,
  channel,
  startTime,
  endTime,
...
dahua.prototype.nextFileFind = function (objectId, count) {
  ...
  request(
    BASEURI +
      "/cgi-bin/mediaFileFind.cgi?action=findNextFile&object=" +
      objectId +
      "&count=" +
      count,
    function (error, response, body) {
...
dahua.prototype.closeFileFind = function (objectId) {
  ...
  request(
    BASEURI + "/cgi-bin/mediaFileFind.cgi?action=close&object=" + objectId,
    function (error, response, body) {
      if (error || response.statusCode !== 200 || body.trim() !== "OK") {
...
dahua.prototype.destroyFileFind = function (objectId) {
  ...
  request(
    BASEURI + "/cgi-bin/mediaFileFind.cgi?action=destroy&object=" + objectId,
    function (error, response, body) {
      if (error || response.statusCode !== 200 || body.trim() !== "OK") {
...
dahua.prototype.getSnapshot = function (options) {
  ...
  request(BASEURI + "/cgi-bin/snapshot.cgi?" + options.channel, function (
    ...
    .auth(USER, PASS, false)
    .pipe(fs.createWriteStream(path.join(options.path, options.filename)));
  // TBD: file writing error handling
};

/*=====  End of Get Snapshot  ======*/

dahua.prototype.generateFilename = function (
  device,
  channel,
  start,
  end,
